      ...
      for (let x = 0; x < stage.times; x++) {
        for (let i = 0; i < stage.stages.length; i++) {
          ...
          if (stage.times === 1 || x !== stage.times - 1 || i !== stage.stages.length - 1) {
            ...
            stagesToReturn.push({
              ...subStage,
              originalFrom: oldCurrentTime,
              originalTo: currentTime,
            });
          }
        }
      }
...
export default function calculateStages(stages: (Stage | MultiplyStage)[]) {
  ...
  for (let i = 0; i < stagesWithOriginalTimes.length; i++) {
    ...
    if (stage.type === 'sprint') {
      const newOriginalFromTimespan =
        i == 0 ? stage.originalFrom : stage.originalFrom.subtract(Timespan.fromSeconds(10));
      ...
      const isNextSegmentSprint =
        stagesWithOriginalTimes.length - 1 === i
          ? false
          : stagesWithOriginalTimes[i + 1].type === 'sprint';
      const newOriginalFrom = stage.originalFrom;
      const newOriginalToTimespan = isNextSegmentSprint
        ? stage.originalTo.subtract(Timespan.fromSeconds(10))
        : stage.originalTo;
