...
const MESSAGES: Partial<Record<BluetoothAvailability, string>> = {
  ...
  insecure:
    'Bluetooth only works over a secure (https) connection. Open Runner via its https address.',
  unavailable:
...
// Native (apps/android): "unsupported"/"insecure" can't happen (see bluetoothAvailability.ts),
// and the Chrome-specific wording above doesn't apply — just the OS radio.
const NATIVE_MESSAGES: Partial<Record<BluetoothAvailability, string>> = {
  unavailable: 'Bluetooth is off. Turn it on to connect the treadmill and heart-rate monitor.',
};
...
export default function BluetoothNotice({
  ...
  const message = (isNativeApp() ? NATIVE_MESSAGES : MESSAGES)[availability];
